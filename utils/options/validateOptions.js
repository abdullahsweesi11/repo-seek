...
import confirmOverwrite, { getFilePath } from "./confirmOverwrite.js";
...
function validateStars(argv) {
	...
	if (
		starsMinPresent &&
		starsMaxPresent &&
		argv["stars-min"] > argv["stars-max"]
	)
		throw new Error("--stars-min cannot be greater than --stars-max");
}
...
function isValidDate(date) {
	...
	if (
		generatedDate.getUTCFullYear() !== components[0] ||
...
function validateCreated(argv) {
	...
	if (
		createdAfterPresent &&
		(!isValidDate(argv["created-after"]) ||
			new Date(argv["created-after"]) > new Date())
	)
